...
"""
Best Fit Memory Placement (menu driven, structured like Worst Fit)

This program implements the Best Fit placement:
- For each process, choose the smallest free block that can hold it.
...
def best_fit(blocks, processes):
    ...
    for i, psize in enumerate(processes):
        ...
        for j, free in enumerate(memory):
            if free >= psize and free < best_size:
                best_size = free
                best_idx = j
        if best_idx != -1:
            block_size_before[i] = memory[best_idx]
            memory[best_idx] -= psize
            allocation[i] = best_idx
            remaining_after_alloc[i] = memory[best_idx]
        else:
            allocation[i] = -1
...
def main_menu():
    while True:
        ...
        if choice == "1":
            ...
            if blocks is None:
                continue
            allocation, remaining_after_alloc, final_memory, block_size_before = best_fit(blocks, procs)
