...
public class FirstFit {
    ...
    public FirstFit(int[] blocks, int[] processes) {
            ...
            for (int j = 0; j < memoryBlocks.length; j++) {
                if (memoryBlocks[j] >= processSizes[i]) {
                    ...
                    System.out.println("Process " + (i + 1) + " (" + processSizes[i] +
                            " KB) allocated to Block " + (j + 1) +
                            ". Remaining space: " + memoryBlocks[j] + " KB");
                    break;
                }
            }
            if (!allocated[i]) {
    ...
    public static void main(String[] args) {
        ...
        FirstFit manager = new FirstFit(memoryBlocks, processes);
