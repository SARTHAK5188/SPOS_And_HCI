...
public class BestFit {
    ...
    public BestFit(int[] blocks, int[] processes) {
    ...
    public void allocateMemory() {
        ...
        for (int i = 0; i < processSizes.length; i++) {
            ...
            // Find the best fit block (smallest sufficient block)
            ...
            if (bestBlock != -1) {
                ...
                memoryBlocks[bestBlock] -= processSizes[i];
                allocated[i] = true;
                System.out.println("Process " + (i + 1) + " (" + processSizes[i] +
                        " KB) allocated to Block " + (bestBlock + 1) +
                        " (Best Fit). Remaining: " + memoryBlocks[bestBlock] + " KB");
            } else {
                System.out.println("Process " + (i + 1) + " (" + processSizes[i] +
    ...
    public static void main(String[] args) {
        ...
        BestFit manager = new BestFit(memoryBlocks, processes);
